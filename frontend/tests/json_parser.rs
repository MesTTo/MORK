...
impl Transcriber for EventTranscriber {
    ...
    fn descend_key(&mut self, k: &str, _first: bool) {
        self.events.push(format!("desc:{k:?}"));
    }

    fn ascend_key(&mut self, k: &str, _last: bool) {
        self.events.push(format!("asc:{k:?}"));
    }
...
fn parse_error(input: &str) -> Error {
    ...
    let mut transcriber = EventTranscriber::default();
    parser.parse(&mut transcriber).unwrap_err()
}

#[test]
fn escaped_strings_keep_buffer_backed_keys_stable() {
    let mut parser = Parser::new(r#"{"alpha\nkey": "first\tvalue", "beta\u0041key": "second"}"#);
    let mut transcriber = EventTranscriber::default();
...
fn empty_input_reports_unexpected_end() {
    assert_eq!(parse_error(""), Error::UnexpectedEndOfJson);
}
