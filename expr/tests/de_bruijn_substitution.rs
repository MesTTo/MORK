...
#[test]
#[should_panic(expected = "MORK VarRef index exceeds six-bit encoding")]
fn de_bruijn_substitution_rejects_shifted_reference_that_exceeds_encoding() {
    let mut template = parse!("[3] body $ $");
    let mut first = parse!("$");
    let mut second = parse!("_64");

    substitute(&mut template, &mut [&mut first, &mut second]);
}
