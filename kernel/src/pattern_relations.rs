...
impl PatternRelationRow {
    /// Returns the term bound to a user-visible slot.
    ///
    /// Rows are emitted with `user_bindings` sorted by slot, so this uses a
    /// binary search instead of scanning every binding for every projection
    /// column.
    pub fn user_binding(&self, slot: u8) -> Option<TermId> {
        self.user_bindings
            .binary_search_by_key(&slot, |&(binding_slot, _)| binding_slot)
            .ok()
            .map(|index| self.user_bindings[index].1)
    }
}
...
///
/// The lowered plan is used by the expression-trie exact matcher and by the
/// pattern factor inside `BindingSidecarPlan`. It keeps the canonical `TermId`
/// shape explicit as `App_n(parent, children...)` atoms while PathMap byte
...
/// Functional-dependency-aware variable order for a lowered pattern: a top-down
/// breadth-first order from the root, so every application parent precedes its
/// children.
///
/// Each `App_n(parent, children)` atom carries the functional dependency that the
/// parent term determines its children (a term has a unique structure), so once a
/// parent is bound its children are fixed. Binding variables in this order means
/// each one is functionally determined by an already-bound variable, which is the
/// relational e-matching order that beats the AGM worst case by exploiting the
/// id-to-children dependency (Zhang, Wang, Willsey, Tatlock, "Relational
/// E-matching", POPL 2022). Any variable unreachable from the root is appended.
pub fn fd_variable_order(plan: &PatternRelationPlan) -> Vec<PlanVariable> {
    use std::collections::{BTreeMap, BTreeSet, VecDeque};

    let mut children_of: BTreeMap<PlanVariable, Vec<PlanVariable>> = BTreeMap::new();
    for atom in plan.atoms() {
        let entry = children_of.entry(atom.parent).or_default();
        for child in atom.children.iter() {
            if let PlanValue::Variable(variable) = child {
                entry.push(*variable);
            }
        }
    }

    let mut order = Vec::with_capacity(plan.variables().len());
    let mut seen = BTreeSet::new();
    let mut queue = VecDeque::new();
    if let PlanValue::Variable(root) = plan.root() {
    ...
    while let Some(variable) = queue.pop_front() {
        ...
        order.push(variable);
        if let Some(children) = children_of.get(&variable) {
            for &child in children {
                queue.push_back(child);
            }
        }
    }
    for record in plan.variables() {
...
///
/// This is the exact sidecar matcher used by expression-trie and BindingSpace
/// pattern factors. It uses canonical `TermId` equality only and never invokes
/// general unification.
pub fn match_facts(
    sidecar: &TermIdentitySidecar,
...
mod tests {
    ...
    fn sample_user_binding_row() -> PatternRelationRow {
        PatternRelationRow {
            root: TermId(99),
            user_bindings: vec![(1, TermId(10)), (7, TermId(70)), (63, TermId(630))]
                .into_boxed_slice(),
        }
    }

    fn lower_test_pattern(
        pattern: &[u8],
    ...
    #[test]
    fn pattern_relation_row_finds_sorted_user_binding_by_slot() {
        let row = sample_user_binding_row();

        assert_eq!(row.user_binding(7), Some(TermId(70)));
    }

    #[test]
    fn pattern_relation_row_returns_none_for_missing_slot() {
        let row = sample_user_binding_row();

        assert_eq!(row.user_binding(8), None);
    }
    ...
    fn repeated_variable_pattern_lowers_to_shared_user_slot() {
        ...
        let (sidecar, plan, root_variable) = lower_test_pattern(&pattern);
    ...
    fn ground_application_uses_constants_for_children() {
        ...
        let (_sidecar, plan, root_variable) = lower_test_pattern(&pattern);
        let root_atom = plan
            .atoms()
            .iter()
            .find(|atom| atom.parent == root_variable)
    ...
    #[test]
    fn fd_variable_order_is_top_down_from_root() {
        // edge(f($x), g($y)): the root determines the two inner subterms, which in
        // turn determine the user variables x and y.
        let pattern = app(&[
            sym(b"edge"),
            app(&[sym(b"f"), var()]),
            app(&[sym(b"g"), var()]),
        ...
        let (_sidecar, plan, root_variable) = lower_test_pattern(&pattern);
        let order = fd_variable_order(&plan);

        assert_eq!(order[0], root_variable, "the root is bound first");
        assert_eq!(
            order.len(),
            plan.variables().len(),
            "every variable is ordered"
        );

        // Every application parent precedes each of its variable children.
        for atom in plan.atoms() {
            let parent_pos = order.iter().position(|&v| v == atom.parent).unwrap();
            for child in atom.children.iter() {
                if let PlanValue::Variable(child) = child {
                    let child_pos = order.iter().position(|&v| v == *child).unwrap();
                    assert!(parent_pos < child_pos, "a parent must precede its child");
                }
            }
        }
    }
...
"#,
        ...
        assert!(
            after.general_unifications >= before.general_unifications + product_count,
            "current product path should still invoke general unification"
        );
    }
}
