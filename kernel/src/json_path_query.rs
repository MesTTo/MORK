...
use mork_frontend::bytestring_parser::Parser as BytestringParser;
...
/// and escaped bracketed strings are intentionally rejected here. The current
/// goal is to map exact child selectors onto the expression paths produced by
/// `Space::load_json`, not to implement all of RFC 9535.
...
impl Space {
    ...
    ///
    /// The returned bytes are suitable for `TermIdentitySidecar::insert_term`
    /// and for `BindingAccessPlan::Pattern`. For example,
    /// `$.profile.scores[1]` becomes the encoded equivalent of:
    ///
    /// ```text
    /// (profile (scores (1 $)))
    /// ```
