...
fn clear_dense_output_prefix(wz: &mut WriteZipperTracked<'_, '_, ()>) -> bool {
    ...
    let removed_paths = {
        ...
        while rz.to_next_val() {
            if root_had_value && rz.path().is_empty() {
                continue;
            }
...
fn assert_empty_op_args(op_name: &str, op_args: &[Expr]) {
    assert!(
        op_args.is_empty(),
        "{op_name} tensor-op-f32 op clause must be (op {op_name})"
    );
}

fn parse_layernorm_eps(op_args: &[Expr]) -> f32 {
    assert_eq!(
        op_args.len(),
        1,
        "layernorm tensor-op-f32 op clause must be (op layernorm eps)"
    );
    let eps = symbol_f32(op_args[0]);
    assert!(
        eps.is_finite() && eps >= 0.0,
        "layernorm tensor-op-f32 eps must be a finite non-negative f32"
    );
    eps
}

fn last_axis_len(op_name: &str, shape: &[usize]) -> usize {
    assert!(
        !shape.is_empty(),
        "{op_name} tensor-op-f32 input must have rank >= 1"
    );
    let dim = shape[shape.len() - 1];
    assert!(
        dim > 0,
        "{op_name} tensor-op-f32 last axis must be non-empty"
    );
    dim
}

fn infer_add_output_shape(input_decls: &[Expr]) -> Vec<usize> {
    assert_eq!(
        input_decls.len(),
        2,
...
fn infer_layernorm_output_shape(input_decls: &[Expr]) -> Vec<usize> {
    assert_eq!(
        input_decls.len(),
        3,
    ...
    assert_eq!(
        ...
        &[hidden],
        "layernorm tensor-op-f32 G shape must be [D]"
    );
    assert_eq!(
        beta_shape.as_slice(),
        &[hidden],
        "layernorm tensor-op-f32 B shape must be [D]"
    );
    x_shape
}
...
fn validate_reshape_shapes(input_shape: &[usize], output_shape: &[usize]) {
    let input_count = shape_element_count(input_shape);
    let output_count = shape_element_count(output_shape);
    assert_eq!(
        input_count, output_count,
        "reshape tensor-op-f32 element count mismatch: input shape {input_shape:?} has {input_count} elements, output shape {output_shape:?} has {output_count} elements"
    );
}

fn infer_reshape_output_shape(input_decls: &[Expr], output: &TensorOpOutputDecl) -> Vec<usize> {
    assert_eq!(
        input_decls.len(),
        1,
    ...
    let output_shape = output
        .shape
        .clone()
        .unwrap_or_else(|| panic!("reshape tensor-op-f32 requires an explicit output shape"));
    validate_reshape_shapes(&input_shape, &output_shape);
    output_shape
}
...
fn infer_tensor_op_output_shape(
    ...
    let inferred = match op_name {
        ...
        "add" => {
            assert_empty_op_args("add", op_args);
            infer_add_output_shape(input_decls)
        }
        "layernorm" => {
        ...
        "gelu" => {
            ...
            infer_unary_dense_output_shape("gelu", input_decls)
        }
        "softmax" => {
            assert_empty_op_args("softmax", op_args);
            infer_unary_dense_output_shape("softmax", input_decls)
        }
        "reshape" => {
            assert_empty_op_args("reshape", op_args);
            infer_reshape_output_shape(input_decls, output)
        }
...
struct AddKernel {
    lhs: DenseTensorInput,
    rhs: DenseTensorInput,
}

struct LayerNormKernel {
    x: DenseTensorInput,
    gamma: DenseTensorInput,
    beta: DenseTensorInput,
    eps: f32,
}
...
impl TensorOpF32Plan {
    pub(crate) fn from_syntax(syntax: &TensorOpF32Syntax) -> Self {
        let kernel = match syntax.op_name.as_str() {
            ...
            "add" => {
                assert_empty_op_args("add", &syntax.op_args);
                assert_eq!(
                    syntax.input_decls.len(),
                    2,
                    "add tensor-op-f32 needs two input declarations"
                ...
                assert_eq!(
                    syntax.cell_templates.len(),
                    2,
                    "add tensor-op-f32 needs two cell templates"
                );

                let lhs = build_dense_tensor_input(syntax.input_decls[0], syntax.cell_templates[0]);
                let rhs = build_dense_tensor_input(syntax.input_decls[1], syntax.cell_templates[1]);
                assert_eq!(
                    rhs.shape(),
                    lhs.shape(),
                    "add tensor-op-f32 input shapes must match"
                );
                TensorOpF32Kernel::Add(Box::new(AddKernel { lhs, rhs }))
            }
            "layernorm" => {
                let eps = parse_layernorm_eps(&syntax.op_args);
                assert_eq!(
                    syntax.input_decls.len(),
                    3,
                    "layernorm tensor-op-f32 needs X, G, and B input declarations"
                ...
                assert_eq!(
                    syntax.cell_templates.len(),
                    3,
                    "layernorm tensor-op-f32 needs X, G, and B cell templates"
                );

                let x = build_dense_tensor_input(syntax.input_decls[0], syntax.cell_templates[0]);
                let gamma =
                    build_dense_tensor_input(syntax.input_decls[1], syntax.cell_templates[1]);
                let beta =
                    build_dense_tensor_input(syntax.input_decls[2], syntax.cell_templates[2]);
                let hidden = last_axis_len("layernorm", x.shape());
                assert_eq!(
                    gamma.shape(),
                    &[hidden],
                    "layernorm tensor-op-f32 G shape must be [D]"
                ...
                TensorOpF32Kernel::LayerNorm(Box::new(LayerNormKernel {
                    x,
                    gamma,
                    beta,
                    eps,
            ...
            "gelu" => {
                ...
                assert_eq!(
                    syntax.input_decls.len(),
                    1,
                    "gelu tensor-op-f32 needs one input declaration"
                );
                assert_eq!(
                    syntax.cell_templates.len(),
                    1,
                    "gelu tensor-op-f32 needs one cell template"
                );
            ...
            "softmax" => {
                ...
                assert_eq!(
                    syntax.input_decls.len(),
                    1,
                    "softmax tensor-op-f32 needs one input declaration"
                );
                assert_eq!(
                    syntax.cell_templates.len(),
                    1,
                    "softmax tensor-op-f32 needs one cell template"
                );

                let input =
                    build_dense_tensor_input(syntax.input_decls[0], syntax.cell_templates[0]);
                last_axis_len("softmax", input.shape());
                TensorOpF32Kernel::Softmax(Box::new(SoftmaxKernel { input }))
            }
            "reshape" => {
                assert_empty_op_args("reshape", &syntax.op_args);
                assert_eq!(
                    syntax.input_decls.len(),
                    1,
                    "reshape tensor-op-f32 needs one input declaration"
                ...
                assert_eq!(
                    syntax.cell_templates.len(),
                    1,
                    "reshape tensor-op-f32 needs one cell template"
                );

                let input =
                    build_dense_tensor_input(syntax.input_decls[0], syntax.cell_templates[0]);
                validate_reshape_shapes(input.shape(), &syntax.output_shape);
                TensorOpF32Kernel::Reshape(Box::new(ReshapeKernel { input }))
            }
    ...
    pub(crate) fn sink_cells(&mut self, cells: &[Expr]) {
        match &mut self.kernel {
            ...
            TensorOpF32Kernel::Add(kernel) => {
                let kernel = kernel.as_mut();
                assert_eq!(cells.len(), 2, "add tensor-op-f32 cell count changed");
                kernel.lhs.set(cells[0]);
                kernel.rhs.set(cells[1]);
            }
            TensorOpF32Kernel::LayerNorm(kernel) => {
                let kernel = kernel.as_mut();
                assert_eq!(cells.len(), 3, "layernorm tensor-op-f32 cell count changed");
                kernel.x.set(cells[0]);
            ...
            TensorOpF32Kernel::Softmax(kernel) => {
                let kernel = kernel.as_mut();
                assert_eq!(cells.len(), 1, "softmax tensor-op-f32 cell count changed");
                kernel.input.set(cells[0]);
            }
            TensorOpF32Kernel::Reshape(kernel) => {
                let kernel = kernel.as_mut();
                assert_eq!(cells.len(), 1, "reshape tensor-op-f32 cell count changed");
                kernel.input.set(cells[0]);
            }
        }
    }
    ...
    pub(crate) fn run(&mut self, output_shape: &Vec<usize>) -> Dense<f32> {
        match &mut self.kernel {
            ...
            TensorOpF32Kernel::Add(kernel) => {
                ...
                assert_eq!(
                    output.data.len(),
                    kernel.lhs.tensor.data.len(),
                    "add tensor-op-f32 output shape changed"
                );
                for ((out, &lhs), &rhs) in output
                    .data
                    .iter_mut()
                    .zip(&kernel.lhs.tensor.data)
                    .zip(&kernel.rhs.tensor.data)
                {
                    *out = lhs + rhs;
                }
                output
            ...
            TensorOpF32Kernel::LayerNorm(kernel) => {
                let kernel = kernel.as_ref();
                let mut output = Dense::<f32>::zeros(output_shape.clone());
                let hidden = kernel.gamma.tensor.data.len();
                assert_eq!(
                    output.data.len(),
                    kernel.x.tensor.data.len(),
                    "layernorm tensor-op-f32 output shape changed"
                );
                for (row, out_row) in kernel
                    .x
                    .tensor
                    .data
                    .chunks_exact(hidden)
                    .zip(output.data.chunks_exact_mut(hidden))
                {
                    ...
                    let variance = row
                        ...
                        .sum::<f32>()
                        / hidden as f32;
                    let inv_std = 1.0 / (variance + kernel.eps).sqrt();
                    for ((out, &x), (&gamma, &beta)) in out_row.iter_mut().zip(row).zip(
                        kernel
                            .gamma
                            .tensor
                            .data
                            .iter()
                            .zip(&kernel.beta.tensor.data),
                    ) {
            ...
            TensorOpF32Kernel::Softmax(kernel) => {
                let kernel = kernel.as_ref();
                let mut output = Dense::<f32>::zeros(output_shape.clone());
                let hidden = last_axis_len("softmax", kernel.input.shape());
                for (row, out_row) in kernel
                    .input
                    .tensor
                    .data
            ...
            TensorOpF32Kernel::Reshape(kernel) => {
                ...
                assert_eq!(
                    output.data.len(),
                    kernel.input.tensor.data.len(),
                    "reshape tensor-op-f32 output element count changed"
                );
                output.data.copy_from_slice(&kernel.input.tensor.data);
                output
            }
        }
    }
