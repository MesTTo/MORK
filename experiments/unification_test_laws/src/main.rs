use unification_test_laws::{
    convert_and_add_line_numbers_big_metta, results_to_single_file, unify_with_mork_unifier,
};
...
fn main() {
    ...
    if run_prolog_unifier {
        ...
        let prolog_exit_status = std::process::Command::new("swipl")
            ...
            .args([
                "-O",
                "-g",
                "['../axiom_unify.prolog'], write_all_results_concurrent, halt.",
            ])
            .spawn()
            .unwrap()
            .wait();
    ...
    if collect_results_and_diff {
        ...
        let mork = [env!("CARGO_MANIFEST_DIR"), "/tmp/mork/all_results.metta"]
            .into_iter()
            .collect::<String>();
        let prolog = [env!("CARGO_MANIFEST_DIR"), "/tmp/prolog/all_results.metta"]
            .into_iter()
            .collect::<String>();
