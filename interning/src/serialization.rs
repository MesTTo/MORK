use crate::{MAX_WRITER_THREADS, SYM_LEN, SYMBOL_THREAD_PERMIT_BYTE_POS, SharedMapping, SharedMappingHandle, Slab, Symbol, ThinBytes, bounded_pearson_hash};
use pathmap::{PathMap, morphisms::Catamorphism};
use std::{
  io::{Read, Write},
  path::Path,
};
...
impl SharedMapping {
    ...
    for (n, lock) in locks.iter().enumerate() {
      ...
      reader.into_cata_jumping_side_effect::<Result<(), std::io::Error>, _>(|_, _, _, v, o| {
        if let Some(ptr) = v {
          ...
          count += if sym_len <= i8::MAX as usize { buffer.write(&[!(sym_len as u8)])? } else { buffer.write(&(sym_len as u64).to_be_bytes())? };
  ...
  pub fn deserialize(in_path: impl AsRef<Path>) -> Result<SharedMappingHandle, std::io::Error> {
    ...
    loop {
      let Some((head, tail)) = meta_slice.split_at_checked((2 * u64::BITS / u8::BITS) as usize) else {
        break;
      };
        ...
        while let Some((sym_bytes, to_parse_0)) = to_parse.split_at_checked(SYM_LEN - SYMBOL_THREAD_PERMIT_BYTE_POS) {
          ...
          let (length, to_parse_2) = if (leading_byte as i8).is_negative() {
            let Some((_, to_parse_1)) = to_parse_0.split_at_checked(1) else {
              return Err(std::io::Error::other(concat!("Malformed data, expected length byte, file : ", file!(), ", line : ", line!())));
            };
            ((!leading_byte) as usize, to_parse_1)
          } else {
            let Some((len_bytes, to_parse_1)) = to_parse_0.split_at_checked(8) else {
              return Err(std::io::Error::other(concat!("Malformed data, expected 8 length bytes, file : ", file!(), ", line : ", line!())));
            };
          ...
          let Some((_, [to_parse_3 @ ..])) = to_parse_2.split_at_checked(length) else {
            return Err(std::io::Error::other(concat!("Malformed data, unexpected end', file : ", file!(), ", line : ", line!())));
          };
...
mod test {
  ...
  fn serialize_long() {
    ...
    let path = temp_zip_path("serialize_long.zip");

    handle.serialize(&path).unwrap();
    let load = SharedMapping::deserialize(&path).unwrap();

    core::assert_eq!(handle.to_bytes[0].0.read().unwrap().val_count(), load.to_bytes[0].0.read().unwrap().val_count());
    load.to_bytes[0].0.read().unwrap().read_zipper().into_cata_side_effect(|_mask, _accs, val: Option<&ThinBytes>, _path| {
      match val {
        Some(id) => {
          let bytes = unsafe { &*id.as_raw_slice() };
          core::assert!(bytes.iter().all(|c| c.is_ascii_alphanumeric()));
        }
        None => (),
      };
    });
  ...
  fn trivial_serialize() {
    let path = temp_zip_path("trivial_serialize.zip");
  ...
  fn temp_zip_path(file_name: &str) -> std::path::PathBuf {
    let dir = std::path::PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap()).join(".tmp");
  ...
  unsafe fn as_btree(shared_mapping: &SharedMapping) -> (BTreeMap<String, [u8; 8]>, BTreeMap<[u8; 8], String>) {
    ...
    for each in 0..MAX_WRITER_THREADS {
      for (path, value) in shared_mapping.to_symbol[each].0.read().unwrap().iter() {
        out.insert(String::from_utf8(path.to_vec()).unwrap(), *value);
      }
      for (value, path) in shared_mapping.to_bytes[each].0.read().unwrap().iter() {
        core::assert!(value.len() == SYM_LEN);
        let symbol: [u8; SYM_LEN] = value.try_into().unwrap();
        out2.insert(symbol, String::from_utf8(unsafe { &*path.as_raw_slice() }.to_vec()).unwrap());
      }
    }
